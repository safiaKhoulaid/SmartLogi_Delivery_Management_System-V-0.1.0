 ...
 */
public record Telephone(String codePays, String numero) {

    // Regex simplifiée pour un numéro de téléphone au Maroc (+212 et 9 chiffres de 5 à 8)
    private static final Pattern MAROC_PATTERN = Pattern.compile("^\\+212[5-8][0-9]{8}$");
    private static final String DEFAULT_MAROC_CODE = "+212";
     ...
     * @param numero Le numéro local ou le numéro complet.
     */
    public Telephone {
        Objects.requireNonNull(numero, "Le numéro de téléphone est obligatoire.");
        String cleanNumero = numero.trim().replaceAll("[\\s-()]+", ""); // Nettoyage des espaces et symboles

        if (cleanNumero.isEmpty()) {
        ...
        if (cleanNumero.startsWith("+")) {
            // Le numéro est déjà au format international
            numeroComplet = cleanNumero;
        } else {
            ...
            numeroComplet = determinedCodePays + cleanNumero;
        }

        // 2. Validation métier (on vérifie que c'est un numéro marocain pour SDMS)
        if (!MAROC_PATTERN.matcher(numeroComplet).matches()) {
        ...
        finalCodePays = numeroComplet.substring(0, numeroComplet.length() - 9);
        finalNumero = numeroComplet.substring(numeroComplet.length() - 9);
        ...
        // NOTE: Si vous voulez le comportement de normalisation complet, utilisez this(finalCodePays, finalNumero)
    ...
    // On utilise les accesseurs générés (getCodePays(), getNumero())
    ...
    public String getNumeroComplet() {
        return this.codePays + this.numero;
    }
}
